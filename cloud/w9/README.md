...
## 🛠️ 4. Hướng Dẫn Các Bước Kiểm Thử Chi Tiết & Giải Thích Lệnh
...
1. **Khởi động Minikube `w9` profile:**
   ...
   ```
   * **Mục đích:** Khởi động máy ảo Kubernetes Minikube local với cấu hình tài nguyên đủ mạnh (`4 CPUs`, `6GB RAM`) để chạy mượt mà toàn bộ Prometheus Operator, Argo Rollouts, Maildev và API ứng dụng.

2. **Build Docker images cho API ứng dụng và nạp vào cụm:**
   ```bash
   # Build image phiên bản gốc (v1)
   docker build -t w9-api:1 cloud/w9/app/
   # Tạo tag cho phiên bản v2 và v3 từ cùng một codebase (để thay đổi config runtime qua env)
   docker tag w9-api:1 w9-api:2
   docker tag w9-api:1 w9-api:3

   # Nạp các image trực tiếp vào cụm Minikube (không cần push lên registry ngoài)
   minikube image load w9-api:1 -p w9
   minikube image load w9-api:2 -p w9
   minikube image load w9-api:3 -p w9
   ```
   * **Mục đích:** Đảm bảo cụm Kubernetes có sẵn các image `w9-api:1`, `w9-api:2`, `w9-api:3` để chạy thử nghiệm các trạng thái canary.

### 🚀 Bước 1: Khởi tạo GitOps Pipeline (ArgoCD)

1. Thêm các file manifests vào Git, commit và push lên GitHub nhánh `main`.
2. **Apply root application (chỉ cần làm duy nhất một lần):**
   ```bash
   kubectl apply -f cloud/w9/argocd/root.yaml
   ```
   * **Mục đích:** Khai báo ứng dụng **Root App (App-of-Apps)** cho ArgoCD. Từ đây, ArgoCD tự động tiếp quản, quét thư mục Git để tự deploy các ứng dụng con (`api`, `argo-rollouts`, `kube-prometheus-stack`).
...
**Bắt đầu tạo traffic giả lập truy cập vào API:**
```bash
kubectl -n demo run load --image=busybox --restart=Never -- sh -c "while true; do wget -qO- api:8080/; sleep 0.1; done"
```
* **Mục đích:** Chạy một Pod tên là `load` liên tục gửi các HTTP request tới dịch vụ `api` cứ mỗi `0.1 giây`. Lệnh này cực kỳ quan trọng để Prometheus liên tục thu thập metrics chất lượng dịch vụ (SLI/SLO) để vẽ đồ thị và cho `AnalysisRun` tự động đánh giá.

### 🚀 Bước 3: Port-Forward Tiện Ích Ra Máy Cá Nhân

**Mở các terminal mới để port-forward Prometheus và Maildev:**
```bash
# Terminal 1: Port-forward Prometheus Web UI để xem đồ thị và trạng thái Alert
kubectl -n monitoring port-forward svc/kube-prometheus-stack-prometheus 9090:9090

# Terminal 2: Port-forward Maildev Web UI để xem hòm thư email cảnh báo gửi tới
kubectl -n demo port-forward svc/maildev 1080:1080
```
* **Mục đích:** Chuyển tiếp cổng các dịch vụ nằm bên trong cụm Kubernetes ra máy local để bạn có thể mở bằng trình duyệt:
...
2. **Commit và push lên GitHub:**
   ```bash
   git add cloud/w9/k8s/api.yaml
   git commit -m "deploy: upgrade API to v2"
   git push origin main
   ```
   * **Mục đích:** Thay đổi cấu hình phiên bản mới trên Git (GitOps source of truth). ArgoCD sẽ tự động đồng bộ và Argo Rollouts bắt đầu quá trình Canary.
3. Vì phiên bản `v2` tốt (tỉ lệ thành công 100%), bạn sẽ thấy `AnalysisRun` đo lường thành công nền. Trọng số traffic tự động tăng dần từ `25%` (1 pod v2, 3 pod v1) ➜ `50%` (2 pod v2, 2 pod v1) ➜ `100%` (4 pod v2) mà không cần can thiệp thủ công.
...
2. **Commit và push lên GitHub:**
   ```bash
   git add cloud/w9/k8s/api.yaml
   git commit -m "deploy: upgrade API to v3 (failing version)"
   git push origin main
   ```
   * **Mục đích:** Đẩy phiên bản lỗi `v3` lên Git để kích hoạt Canary.
3. ArgoCD đồng bộ và scale-up 1 pod v3 (nhận 25% traffic). Pod v3 lỗi 50% số request dẫn đến tỉ lệ thành công tổng thể của dịch vụ giảm xuống **~87%** (nhỏ hơn SLO 95%).
4. **Quan sát trạng thái phân tích tự động:**
   ```bash
   kubectl get analysisrun -n demo
   ```
   * **Mục đích:** `AnalysisRun` sẽ kiểm tra success rate mỗi 15 giây. Sau 5 lần đo bị lỗi liên tiếp (vượt quá `failureLimit: 5`), quá trình Canary tự động hủy bỏ (**Auto-abort**). Pod v3 bị xóa bỏ, cụm tự động khôi phục scale-up bản ổn định `v2` trở lại 100%.
...
3. Alertmanager tự động gửi email cảnh báo tới SMTP Maildev với tiêu đề:
   ...
   * **Mục đích:** Xác minh hệ thống cảnh báo hoạt động chính xác khi có sự cố SLO xảy ra.
...
git push origin main
```
* **Mục đích:** Cập nhật lại GitOps source of truth về phiên bản ổn định `v2`. ArgoCD đồng bộ lại hoàn chỉnh trạng thái, đưa toàn bộ hệ thống về trạng thái xanh, sạch và đồng nhất hoàn toàn.
